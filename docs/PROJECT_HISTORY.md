...
6. האם יישור סשן (session alignment) בין קובץ האימון T לקובץ ההערכה E יכול להעלות את kappa, במיוחד בנבדקים החלשים?
...
- `riemann_ea` / `riemann_ra`: יישור סשן (Euclidean/Riemannian Alignment) לפני tangent space
- `riemann_ea_fbcsp_vote`: הגרסה המיושרת של ה-ensemble הטוב ביותר
- חלונות זמן שונים:
...
### יישור סשן בין T ל-E (Session Alignment)

בפרוטוקול train-T/test-E, קובץ האימון (`T`) וקובץ ההערכה (`E`) מוקלטים בסשנים נפרדים, ולכן התפלגות ה-covariance של הערוצים "זזה" בין הסשנים (session drift). ההיסט הזה פוגע בהעברה מ-T ל-E, במיוחד בנבדקים ה"רועשים" (A02, A05, A06) שקיבלו kappa נמוך.

כדי לתקן זאת הוספנו יישור סשן בשיטת Euclidean Alignment (He & Wu, 2020): לפני המעבר ל-tangent space, ממרכזים את מטריצות ה-covariance של כל סשן בנפרד כך שהממוצע שלהן הופך למטריצת היחידה. היישור מחושב באופן טרנסדוקטיבי — reference נפרד נאמד על T ו-reference נפרד על E — ולכן הוא מבטל את ההיסט הספציפי לסשן בלי להשתמש בתוויות ההערכה. היישור מיושם בנפרד בכל תת-תחום תדר של בנק התדרים.

מודלים חדשים שנוספו:

```text
riemann_ea             Riemannian tangent space + Euclidean Alignment
riemann_ra             יישור לפי ממוצע רימני (Riemannian Alignment)
riemann_ea_fbcsp_vote  הגרסה המיושרת של ה-ensemble הטוב ביותר
```

תוצאות ה-benchmark (train-T/test-E על 9 הנבדקים):
...
היישור עזר: `riemann_ea` לבדו השיג mean kappa = 0.647, כלומר מודל בודד מיושר שווה ל-ensemble הקודם. ה-ensemble המיושר `riemann_ea_fbcsp_vote` הגיע ל-mean kappa = 0.670 — התוצאה הטובה ביותר בפרויקט, שיפור של 0.023 מעל הקודם.

תוצאות לפי נבדק (kappa), ה-ensemble המיושר מול ה-ensemble הקודם:

```text
subject   vote     ea_vote   שינוי
...
היישור שיפר 7 מתוך 9 נבדקים, כולל שניים מהנבדקים החלשים (A02, A06). A05 נשאר עקשן. זה מאשר את ההשערה שחלק ניכר מהפער בין T ל-E נובע מהיסט סשן שאפשר לתקן באופן טרנסדוקטיבי, בלי להשתמש בתוויות ההערכה.
...
בדקנו גם האם אימון מודל אחד על כל קובצי ה-T של כל 9 הנבדקים יחד ישפר את התוצאה. האינטואיציה הייתה שיותר נתונים אולי יעזרו, אבל בפועל זה פגע משמעותית בביצועים:

```text
subject-specific riemann_fbcsp_vote   mean kappa = 0.647
pooled riemann_fbcsp_vote             mean kappa = 0.360
```

זה מחזק את ההבנה שדפוסי EEG שונים מאוד בין נבדקים. במאגר הזה, כיול אישי לכל נבדק עדיף בהרבה על pooling פשוט. אם רוצים לנצל מידע מנבדקים אחרים, כנראה צריך domain adaptation, transfer learning, או fine-tuning אישי ולא ערבוב פשוט של כל הנתונים.
...
### ניסויים אחרי היישור שלא שיפרו

לאחר שהיישור (EA) העלה את התוצאה ל-0.670, בדקנו שני כיוונים נוספים, ושניהם לא שיפרו:

- **Stacking meta-learner**: במקום soft-vote, אימנו מסווג לוגיסטי מעל הסתברויות ה-base decoders (עם CV פנימי). התוצאה יצאה שקולה ל-vote (ההפרשים בין הנבדקים בתוך תחום הרעש), ולכן לא שווה את העלות החישובית הגבוהה.
- **Crop augmentation**: אימון על חלונות משנה חופפים של 2 שניות מכל trial (פי 4 דגימות אימון) ומיצוע ההסתברויות לכל trial. זה דווקא הוריד את ה-kappa ל-0.562 ופגע בכל 9 הנבדקים. הסיבה: crop augmentation עוזר לרשתות נוירונים, אבל מטריצת covariance היא סטטיסטיקה גלובלית שדורשת את כל ה-trial; חלונות קצרים יותר נותנים אומדני covariance רועשים יותר. כלומר מה שעוזר ל-deep learning פוגע דווקא בשיטות Riemannian.
- **Riemannian Alignment (RA)**: יישור לפי ממוצע גאומטרי (רימני) במקום ממוצע אוקלידי (EA). התוצאה זהה ל-EA (`riemann_ra_fbcsp_vote` = 0.670, `riemann_ra` = 0.647), ולכן EA עדיף כי הוא פשוט ומהיר יותר.

מסקנה: מבין כל השיפורים שנוסו אחרי ה-baseline, רק יישור סשן (EA) עזר. ה-kappa הטוב ביותר נשאר 0.670.
...
**יישור מקוון (online EA):** היישור הרגיל (EA) עובד על batch של כל קובץ ההערכה בבת אחת, ולכן אינו תקף לחיזוי חלון-בודד בזרם חי. כדי לאפשר יישור גם ב-live, המימוש ב-`live_demo` מריץ כל חיזוי על ה-batch של החלונות האחרונים (`--align-windows`, ברירת מחדל 20), כך שהיישור מחושב מ-reference מתגלגל שעוקב אחרי הסחף בזמן אמת בלי צורך בתוויות. עבור decoder לא-מיושר התוצאה זהה לחיזוי חלון-בודד, ולכן זה בטוח לכל המודלים. זהו הצעד הראשון לכיוון הסתגלות רציפה בסגנון מערכות invasive.
...
לכן הפרויקט מראה שחזור של baseline קלאסי חזק, שיפור בעזרת Riemannian geometry, שיפור נוסף בעזרת ensemble, ולבסוף שיפור נוסף בעזרת יישור סשן (Euclidean Alignment) שמתקן את ההיסט בין T ל-E.
